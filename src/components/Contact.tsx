...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-20 lg:py-28 bg-slate-50 relative overflow-hidden">
      <div className="absolute top-20 right-0 w-72 h-72 bg-blue-100 rounded-full blur-3xl opacity-50"></div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {contactInfo.map((info) => (
                <a
                  key={info.label}
                  href={info.href}
                  className="group bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-slate-100"
                >
                  <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-blue-600 to-green-600 mb-4 group-hover:scale-110 transition-transform duration-300">
                    <info.icon size={22} className="text-white" />
                  </div>
                  <div className="text-slate-500 text-sm font-medium mb-1">{info.label}</div>
                  <div className="text-slate-900 font-semibold text-sm break-words">{info.value}</div>
                </a>
              ))}
            </div>
